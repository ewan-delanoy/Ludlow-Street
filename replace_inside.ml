...
let overwrite_at_intervals_inside_string ovw_b l_itvs s=
  List.fold_left(fun t (i,j)->overwrite_at_interval_inside_string ovw_b (i,j) t)
  s l_itvs;;

let overwrite_at_intervals_inside_file ovw_b l_itvs fn=
  let old_t=Io.read_whole_file fn in
  let new_t=overwrite_at_intervals_inside_string ovw_b l_itvs old_t in
