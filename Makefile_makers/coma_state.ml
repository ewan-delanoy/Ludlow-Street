...
let recompile x=
     ...
     let (new_mdata2,new_tgts2,rejected_ones2)=
       ...
       (get_root x) new_mdata new_tgts  in
...
let backup x diff opt=
  ...
  (get_root x,get_backup_dir x) diff opt;;

let forget_file x ap=
    let (new_mdata,new_dirs,new_tgts)= 
     German_forget_file.on_targets (get_root x)
...
let forget_module x hm=
    let ((new_mdata,new_dirs,new_tgts),short_paths)= 
      German_forget_module.on_targets (get_root x)
...
let initialize x=
        let s_ap=Root_directory.join (get_root x)  Coma_constant.name_for_targetfile in
...
let refresh x=
      let (new_mdata,new_tgts,new_ptypes)=
        ...
        let new_diff=German_delchacre_from_scratch.dfs (get_root x,get_backup_dir x) new_mdata in
        let _=
        (
          set_data x new_mdata;
          set_directories x new_dirs;
...
let relocate_module x old_name new_subdir=
  let ((new_mdata,new_tgts),(old_files,new_files))=
    German_relocate_module.on_targets (get_root x)
...
let rename_directory x (old_subdir,new_subdirname)=
      let _=Rename_endsubdirectory.in_unix_world 
       (get_root x) (old_subdir,new_subdirname) in
...
let rename_module x old_name new_name=
      let ((new_mdata,new_tgts),(old_files,new_files))=
        German_rename_module.on_targets (get_root x)
...
let start_debugging x=
          let (bowl,(new_mdata,new_tgts,_))=
            German_start_debugging.sd (get_root x)
...
let unregister_mlx_file x mlx=
        let (new_mdata,new_dirs,new_tgts)= 
          German_unregister_mlx_file.on_targets (get_root x)
...
let unregister_module x hm=
        let ((new_mdata,new_dirs,new_tgts),short_paths)= 
         German_unregister_module.on_targets (get_root x)
