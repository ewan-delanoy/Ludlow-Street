...
#use"Makefile_makers/md_list.ml";;

Operations on the md_list_t datatype

*)

let empty_one=Md_list_t.L [];;

let industrial_separator=Industrial_separator.alaskan_data;; 

let archive (Md_list_t.L mdata)=
...
let unarchive s=
  ...
  Md_list_t.L(Image.image Modulesystem_data.unarchive v1);;

let compute_subdirectories_list (Md_list_t.L mdata)=
...
let find_needed_data_for_file (Md_list_t.L mdata) fn=
...
let check_presences (Md_list_t.L mdata) hm=
...
let complete_info wmdata  mlx=
  ...
  let libned=Private.find_needed_libraries mlx genealogy
  and dirned=Private.find_needed_directories mlx genealogy in
...
let quick_update mdata x=
      ...
      let mlx=Mlx_ended_absolute_path.join hm edg in
      let fathers=Private.find_needed_names mdata mlx in
      Some(
      {
...
module Private_for_ancs_libs_and_dirs=struct      
      ...
      let iterator_for_update (graet,da_ober)=match da_ober with
        ...
        |(md,atoms_for_md)::peurrest->
           ...
           let new_libs=Private.find_needed_libraries mlx genealogy
           and new_dirs=Private.find_needed_directories mlx genealogy in
           let new_md=
           {
...
let debuggable_targets_from_ancestors mdata ancestors=
  let temp1=Image.image (fun hm2->
         ...
         PrivateTwo.debuggable_targets_from_ancestor_data dt2
       ) ancestors in
...
let ingredients_for_debuggable wmdata hm=
    ...
    @(PrivateTwo.immediate_ingredients_for_debuggable hm);;    
...
let register_mlx_file_on_monitored_modules wmdata mlx_file =
          ...
          let (before,opt,after)=Three_parts.select_center_element  (fun dt->
             ...
             mdata in
          if opt=None
          then  let old_info=complete_info wmdata mlx_file in
          ...
          then Md_list_t.L(before@(new_dt::after)) 
          else 
...
module PrivateThree=struct
            ...
            let put_md_list_back_in_order tolerate_cycles 
              ...
              (Md_list_t.L(final_list),active_descendants);;
...
let printer_equipped_types_from_data (Md_list_t.L mdata)=
...
let unregister_module_on_monitored_modules (Md_list_t.L mdata) hm=
      let desc=List.filter(
       ...
       (Md_list_t.L(before@after),short_paths);;    
...
let unregister_mlx_file_on_monitored_modules (Md_list_t.L mdata) mlxfile=
        ...
        let desc=List.filter(
          ...
          |Some(dt)->
            ...
            then Md_list_t.L(before@after)
            else Md_list_t.L(before@(new_dt::after));;
...
let recompute_module_info wmdata hm=
     ...
     let (before,_,after)=Three_parts.select_center_element(
      ...
      let new_md=recompute_complete_info_for_module wmdata hm in 
      Md_list_t.L(before@(new_md::after));;            

      exception Nonregistered_module of Half_dressed_module.t;;  

let descendants (Md_list_t.L mdata) names=
...
let rename_module_on_monitored_modules wmdata old_name new_name=
         let (Md_list_t.L mdata)=wmdata in
         ...
         let desc=descendants wmdata [old_name] in
         ...
         (Md_list_t.L(new_list),(old_files,new_files));;
...
let relocate_module_on_monitored_modules wmdata old_name new_subdir=
            let (Md_list_t.L mdata)=wmdata in
            let (before,opt,after)=Three_parts.select_center_element  (fun dt->
            ...
            let part2=Image.image data_renamer (old_dt::after) in
                 (Md_list_t.L(before@part2),(old_files,new_files));;

let above (Md_list_t.L mdata) hm=
...
let all_short_paths (Md_list_t.L mdata)=List.flatten(
...
let insertion_index wmdata lower_bound upper_bound=
    let (Md_list_t.L mdata)=wmdata in
    if upper_bound=None
...
let insert_data wmdata x (l_hm_before,l_hm_after)=
  let (Md_list_t.L mdata)=wmdata in
  ...
  and upper_bound= (if l_idx_after=[]  then None else 
   ...
   let i=insertion_index wmdata lower_bound upper_bound in
   ...
   Md_list_t.L(temp3@(x::temp4));;
  
exception Nonregistered_module_during_reposition of Half_dressed_module.t;;  
  
   
let reposition_module (Md_list_t.L mdata) hm (l_before,l_after)=
      ...
      insert_data 
       (Md_list_t.L amputated_data) info (l_before,l_after);;  

let rename_directory_on_data (old_subdir,new_subdirname) (Md_list_t.L mdata)=
  Md_list_t.L(Image.image (Modulesystem_data.rename_endsubdirectory (old_subdir,new_subdirname)) mdata);;

let find_value_definition (Md_list_t.L mdata) s=
...
let find_naked_module_registration (Md_list_t.L mdata) nm=
      Option.seek (fun md->
        let hm=Modulesystem_data.name md in
...
let all_naked_modules (Md_list_t.L mdata)=
  Image.image (fun md->
  Naked_module.to_string(
    Half_dressed_module.naked_module(Modulesystem_data.name md))
  ) mdata;;     

let all_ml_absolute_paths (Md_list_t.L mdata)=
...
let modules_using_value (Md_list_t.L mdata) value_name =
