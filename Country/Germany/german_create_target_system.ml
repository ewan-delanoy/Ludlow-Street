...
#use"Country/Germany/german_create_target_system.ml";;
...
let select_good_files ()=
   let main_dir=German_constant.root in
   let s_main_dir=Directory_name.to_string main_dir in
   let main_ap=Absolute_path.of_string s_main_dir in
   let _=Sys.command ("touch "^s_main_dir^(Debugger_name.debugger_name)^".ml") in
   let temp1=More_unix.complete_ls main_ap
   and n1=String.length(s_main_dir) in
   let selector=(
   fun ap->
...
let from_prepared_list mdata l=
   let dir=German_constant.root in
   let temp1=Option.filter_and_unpack (fun (ap,s)->
      Mlx_filename.try_from_path_and_root ap dir
   ) l in
   German_modify_modulesystem.try_to_register_mlx_files mdata temp1;;

let from_main_directory ()=
	let temp1=select_good_files () in
    let temp2=clean_list_of_files temp1 in
    let temp3=compute_dependencies temp2 in
   let (failures,mdata1)=from_prepared_list [] temp3 in
   let tl1=German_data.default_toplevel mdata1 in
   let (bowl,(mdata2,tgts))=German_make_ocaml_target.make (mdata1,[]) tl1 in
   let printer_equipped_types=German_printer_equipped_types.from_data mdata2 in
   (bowl,(mdata2,tgts,printer_equipped_types));;
