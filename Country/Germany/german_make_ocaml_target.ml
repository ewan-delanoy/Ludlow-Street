...
#use"Country/Germany/german_make_ocaml_target.ml";;
...
let is_up_to_date tgts tgt=
  ...
  if Ocaml_target.test_path German_constant.root tgt
  then List.mem tgt tgts
  else false;;

let unit_make (bowl,(mdata,tgts)) tgt=
  if (not bowl)
  then (bowl,(mdata,tgts))
  else
  if is_up_to_date tgts tgt
  ...
  then let opt_tgt=(if Ocaml_target.is_a_debuggable tgt then None else (Some tgt)) in
       ...
       |Some(mlx)->
                   let mdata2=German_modify_modulesystem.force_modification_time_update mdata mlx in
                   (true,(mdata2,tgts2))        
  else (false,(mdata,tgts));;

let make_nontoplevel (mdata,tgts) tgt=
  let l=ingr_for_tgt mdata tgt in
  List.fold_left unit_make  (true,(mdata,tgts)) l;;
