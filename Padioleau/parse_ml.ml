...
let rec lexer_function tr = fun lexbuf ->
  ...
  | [] -> (Common.pr2 "LEXER: ALREADY AT END"; tr.PI.current)
  | v::xs -> 
      tr.PI.rest <- xs;
      tr.PI.current <- v;
...
let parse2 filename = 
  ...
  let lexbuf_fake = Lexing.from_function (fun _buf _n -> raise Common.Impossible) in
  ...
  with (Lexer_ml.Lexical _ | Parsing.Parse_error) as exn   ->
    ...
    then begin
      ...
      | Lexer_ml.Lexical s -> 
          Common.pr2 ("lexical error " ^s^ "\n =" ^ error_msg_tok cur)
      | Parsing.Parse_error -> 
          Common.pr2 ("parse error \n = " ^ error_msg_tok cur)
      (* | Semantic_java.Semantic (s, i) -> 
         pr2 ("semantic error " ^s^ "\n ="^ error_msg_tok tr.current)
      *)
      | _e -> raise Common.Impossible
      );
