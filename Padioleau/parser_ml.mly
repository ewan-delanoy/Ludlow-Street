...
let (qufix: Ast_ml.long_name -> Ast_ml.tok -> (string Ast_ml.wrap) -> Ast_ml.long_name) = 
 fun longname dottok ident ->
  match longname with
  | xs, Ast_ml.Name ident2 ->
      xs @ [Name ident2, dottok], Ast_ml.Name ident

let to_item xs =
  xs |> Common.map_filter (function
  | Ast_ml.TopItem x -> Some x
  | _ -> None
  )
%}
/*(*************************************************************************)*/
...
mod_ext_longident:
 ...
 | mod_ext_longident TOParen mod_ext_longident TCParen 
     { [], Ast_ml.Name ("TODOEXTMODULE", $2) }



type_longident:
 | TLowerIdent                               { [], Ast_ml.Name $1 }
 | mod_ext_longident TDot TLowerIdent        { qufix $1 $2 $3 }

val_longident:
 | val_ident                                   { [], Ast_ml.Name $1 }
 | mod_longident TDot val_ident                { qufix $1 $2 $3 }

constr_longident:
 | mod_longident       %prec below_DOT     { $1 }
 | TOBracket TCBracket                     { [], Ast_ml.Name ("[]TODO", $1) }
 | TOParen TCParen                         { [], Ast_ml.Name ("()TODO", $1) }
 | Tfalse                                  { [], Ast_ml.Name ("false", $1) }
 | Ttrue                                   { [], Ast_ml.Name ("true", $1) }
...
seq_expr:
 | expr             %prec below_SEMI   { [Common.Left $1] }
 ...
 /*(* bad ? should be removed ? but it's convenient in certain contexts like
    ...
    *)*/
 | expr TSemiColon                     { [Common.Left $1; Right $2] }
...
*/
 ...
 | Tfor val_ident TEq seq_expr direction_flag seq_expr Tdo seq_expr Tdone
     { For ($1, Ast_ml.Name $2, $3, $4, $5, $6, $7, $8, $9)  }
...
simple_expr:
 ...
 | TOParen seq_expr TCParen
     ...
     | [Common.Right _] -> raise Common.Impossible
     | _ -> Sequence ($1, $2, $3) 
     }
...
expr_comma_list:
 | expr_comma_list TComma expr                  { $1 @ [Common.Right $2; Left $3] }
 | expr TComma expr                             { [Common.Left $1; Right $2; Left $3] }

expr_semi_list:
 | expr                                  { [Common.Left $1] }
 | expr_semi_list TSemiColon expr        { $1 @ [Common.Right $2; Left $3] }
...
lbl_expr_list:
 | label_longident TEq expr
     { [Common.Left (FieldExpr ($1, $2, $3))] }
 | lbl_expr_list TSemiColon     label_longident TEq expr
     { $1 @ [Common.Right $2; Left (FieldExpr ($3, $4, $5))] }
 /*(* new 3.12 feature! *)*/
 | label_longident
      { [Common.Left (FieldImplicitExpr ($1))] }
 | lbl_expr_list TSemiColon     label_longident
     { $1 @ [Common.Right $2; Left (FieldImplicitExpr $3)] }
...
label_expr:
 | TTilde label_ident
      { ArgImplicitTildeExpr ($1, Ast_ml.Name $2) }
 | TQuestion label_ident
      { ArgImplicitQuestionExpr ($1, Ast_ml.Name $2) }
...
match_cases:
 | pattern  match_action                     { [Common.Left ($1, $2)] }
 | match_cases TPipe    pattern match_action { $1 @ [Common.Right $2; Left ($3, $4)] }
...
lbl_pattern_list:
 ...
 | lbl_pattern_list TSemiColon   label_longident TEq pattern 
     { $1 @ [Common.Right $2; Left (PatField ($3, $4, $5))] }
 | lbl_pattern_list TSemiColon   label_longident       
     { $1 @ [Common.Right $2; Left (PatImplicitField ($3))] }
...
pattern_semi_list:
 | pattern                                     { [Common.Left $1] }
 | pattern_semi_list TSemiColon pattern        { $1 @[Common.Right $2; Left $3] }

pattern_comma_list:
 | pattern_comma_list TComma pattern            { $1 @ [Common.Right $2; Left $3] }
 | pattern TComma pattern                       { [Common.Left $1; Right $2; Left $3] }
...
type_declarations:
 | type_declaration                            { [Common.Left $1] }
 | type_declarations Tand type_declaration     { $1 @ [Common.Right $2; Left $3] }
...
type_declaration:
  type_parameters TLowerIdent type_kind /*(*TODO constraints*)*/
   { 
     match $3 with
     | None -> 
         TyAbstract ($1, Ast_ml.Name $2)
     | Some (tok_eq, type_kind) ->
         TyDef ($1, Ast_ml.Name $2, tok_eq, type_kind)
   }
...
constructor_declarations:
 | constructor_declaration                     { [Common.Left $1] }
 | constructor_declarations TPipe constructor_declaration 
     { $1 @ [Common.Right $2; Left $3] }
...
type_parameter_list:
 | type_parameter                               { [Common.Left $1] }
 | type_parameter_list TComma type_parameter    { $1 @ [Common.Right $2; Left $3] }

type_parameter:
  /*(*TODO type_variance*)*/ TQuote ident   { ($1, Ast_ml.Name $2) }



label_declarations:
 | label_declaration                           { [Common.Left $1] }
 | label_declarations TSemiColon label_declaration   { $1 @[Common.Right $2; Left $3]}
...
core_type_comma_list:
 | core_type                                  { [Common.Left $1] }
 | core_type_comma_list TComma core_type      { $1 @ [Common.Right $2; Left $3] }

core_type_list:
  | simple_core_type                         { [Common.Left $1] }
  | core_type_list TStar simple_core_type    { $1 @ [Common.Right $2; Left $3] }
...
let_bindings:
 | let_binding                           { [Common.Left $1] }
 | let_bindings Tand let_binding         { $1 @ [Common.Right $2; Left $3] }
...
opt_bar:
 | /*(*empty*)*/    { [] }
 | TPipe            { [Common.Right $1] }

with_constraints:
 | with_constraint                             { [Common.Left $1] }
 | with_constraints Tand with_constraint        { $1 @ [Common.Right $2; Left $3] }
