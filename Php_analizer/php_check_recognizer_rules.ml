...
let z1=image (fun cst->Php_token.Constant(cst))
    Php_constant_token.all ;;
let part2=image  Php_token.projected_version z1;;

let part3=image fst Php_atomic_selector.special_list;;

let part4=image fst Php_short_selector.new_constants;;

let part5=
   ofo(Tidel.diforchan(
   (image fst Php_constructible_recognizer.all_pairs)
   @
   (image snd Php_constructible_recognizer.all_pairs)
   ));;
