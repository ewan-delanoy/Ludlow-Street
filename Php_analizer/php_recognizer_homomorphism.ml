...
let stepper_for_chain 
  (da_ober,support,nondummy_left_lexings,nondummy_right_lexings,should_exit)=
   ...
   |rcgzr::peurrest->
     (
        match Old_php_recognizer.recognize rcgzr support with
       None->([],[],[],[],true)
      |Some(cr,support2)->
        (peurrest,support2,
...
let zzz_helper_for_ordered_disjunction l=
  let rec tempf=(fun lf->
     match lf with
      []->None
     |rcgzr::peurrest->(
                      match Old_php_recognizer.recognize rcgzr l with
                       None->tempf peurrest
...
let unfinished_star rcgzr=
   ...
   match Old_php_recognizer.recognize rcgzr l3 with
    None->Some(Php_char_range.make u v1,l2)
   |Some(cr3,l4)->tempf(u,v2,l3,Php_char_range.snd cr3,l4)
   ) in 
   let f=(fun l1->
   match Old_php_recognizer.recognize rcgzr l1 with
    None->None
   |Some(cr1,l2)->
        let (u1,v1)=Php_char_range.unveil cr1 in
        (
          match Old_php_recognizer.recognize rcgzr l2 with
