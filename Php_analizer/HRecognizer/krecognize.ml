...
#use"Php_analizer/HRecognizer/krecognize.ml";;
...
let c x y=
    Kregistrar.leaf x (Atomic_hrecognizer.constant y);;
let lc x y=
    Kregistrar.leaf x (Atomic_hrecognizer.later_constant y);;
let st x y=
    Kregistrar.leaf x (Atomic_hrecognizer.star y);;
let sto x y=
  Kregistrar.leaf x (Atomic_hrecognizer.star_outside y);;
let enc x y=
    Kregistrar.leaf x (Atomic_hrecognizer.enclosed y);;  
let sq=Kregistrar.leaf "sq" Atomic_hrecognizer.simple_quoted;;
let dq=Kregistrar.leaf "dq" Atomic_hrecognizer.double_quoted;;           
let ch x l=Kregistrar.chain x l;;   
let dis x l=Kregistrar.ordered_disjunction x l;;   


let star=Kregistrar.star;;
let maybe=Kregistrar.maybe;;
let keyword_avoider=Kregistrar.keyword_avoider;;
...
let for_kwd=kc "for_kwd" "for";;
let foreach_kwd=kc "foreach_kwd" "foreach";;
...
let phoreech_loop=ch
  label_for_phoreech_loop
  [
     foreach_kwd;
     whites;
     paren_block;
...
let semicoloned_paamayim_call=ch 
...
let big_qualified_declaration=ch 
...
let abstract_qualified_fnctn=ch
...
let abstracted_qualified_fnctn=ch
